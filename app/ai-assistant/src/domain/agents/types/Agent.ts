...
export interface ICodeReviewAgent {
  streamCodeProposal(messages: ModelMessage[], metadata: UserMetadata, userId: string): Promise<Response>;
}

export interface ICodeIntegrationAgent {
  generateCodeChanges(proposedChanges: ProposeCodeChange, code: string, userId: string): Promise<any>;
}

export interface IExecutionAnalyzerAgent {
  streamText(messages: ModelMessage[], metadata: UserMetadata, userId: string): Promise<Response>;
}
